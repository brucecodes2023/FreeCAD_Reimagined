...
class TestTopologicalNamingProblem(unittest.TestCase):
    ...
    def testPD_TNPSketchPadSketchBSpline(self):
        ...
        geoList = []
        geoList.append(Part.LineSegment(App.Vector(0, 0, 0), App.Vector(40, 0, 0)))
        geoList.append(Part.LineSegment(App.Vector(40, 0, 0), App.Vector(40, 15, 0)))
        top_spline = Part.BSplineCurve()
        top_spline.interpolate(
            [
                App.Vector(40, 15, 0),
                App.Vector(20, 20, 0),
                App.Vector(0, 15, 0),
        ...
        geoList.append(top_spline)
        geoList.append(Part.LineSegment(App.Vector(0, 15, 0), App.Vector(0, 0, 0)))
        doc.Sketch.addGeometry(geoList, False)
        constraintList = []
        constraintList.append(Sketcher.Constraint("Coincident", 0, 2, 1, 1))
        constraintList.append(Sketcher.Constraint("Coincident", 1, 2, 2, 1))
        ...
        self.assertEqual(doc.Sketch.Geometry[2].TypeId, "Part::GeomBSplineCurve")
        doc.Sketch.moveGeometry(2, 2, App.Vector(0, 2, 0), False)
        doc.recompute()
        self.assertAlmostEqual(doc.Pad.Shape.Volume, 5600.0)
        self.assertTrue(doc.Sketch001.isValid())
        self.assertTrue(doc.Sketch001.AttachmentOffset.Matrix == App.Matrix())
        matrix1 = App.Matrix()
        matrix1.A34 = 10  # Z offset by 10.

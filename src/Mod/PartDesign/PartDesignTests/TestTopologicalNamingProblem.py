...
class TestTopologicalNamingProblem(unittest.TestCase):
    ...
    def testPD_TNPSketchPocketSketchMove(self):
        ...
        doc = App.ActiveDocument
        App.activeDocument().addObject("PartDesign::Body", "Body")
        doc.Body.newObject("Sketcher::SketchObject", "SketchBase")
        doc.SketchBase.AttachmentSupport = (doc.XY_Plane, [""])
        doc.SketchBase.MapMode = "FlatFace"
        geoList = []
        geoList.append(Part.LineSegment(App.Vector(0, 0, 0), App.Vector(40, 0, 0)))
        geoList.append(Part.LineSegment(App.Vector(40, 0, 0), App.Vector(40, 20, 0)))
        geoList.append(Part.LineSegment(App.Vector(40, 20, 0), App.Vector(0, 20, 0)))
        geoList.append(Part.LineSegment(App.Vector(0, 20, 0), App.Vector(0, 0, 0)))
        doc.SketchBase.addGeometry(geoList, False)
        constraintList = []
        constraintList.append(Sketcher.Constraint("Coincident", 0, 2, 1, 1))
        constraintList.append(Sketcher.Constraint("Coincident", 1, 2, 2, 1))
        constraintList.append(Sketcher.Constraint("Coincident", 2, 2, 3, 1))
        ...
        constraintList.append(Sketcher.Constraint("Vertical", 1))
        constraintList.append(Sketcher.Constraint("Vertical", 3))
        doc.SketchBase.addConstraint(constraintList)
        doc.recompute()
        doc.Body.newObject("PartDesign::Pad", "Pad")
        doc.Pad.Profile = (
            doc.SketchBase,
        ...
        doc.Pad.Length = 10
        doc.Pad.ReferenceAxis = (doc.SketchBase, ["N_Axis"])
        doc.SketchBase.Visibility = False
        doc.Pad.Length = 10.000000
        doc.Pad.TaperAngle = 0.000000
        doc.Pad.UseCustomVector = 0
        doc.Pad.Direction = (0, 0, 1)
        ...
        doc.Sketch.addConstraint(constraintList)
        doc.recompute()
        doc.Body.newObject("PartDesign::Pocket", "Pocket")
        doc.Pocket.Profile = (
            doc.Sketch,
            [
                "",
        ...
        doc.Pocket.Length = 5
        doc.Pocket.ReferenceAxis = (doc.Sketch, ["N_Axis"])
        doc.Sketch.Visibility = False
        doc.Pocket.Length = 5.000000
        doc.Pocket.TaperAngle = 0.000000
        doc.Pocket.UseCustomVector = 0
        doc.Pocket.Direction = (0, 0, -1)
        doc.Pocket.ReferenceAxis = (doc.Sketch, ["N_Axis"])
        doc.Pocket.AlongSketchNormal = 1
        doc.Pocket.Type = 0
        doc.Pocket.UpToFace = None
        doc.Pocket.Reversed = 0
        doc.Pocket.Offset = 0
        doc.recompute()
        doc.Sketch.Visibility = False
        doc.Body.newObject("Sketcher::SketchObject", "Sketch001")
        ...
        self.assertTrue(doc.Sketch001.AttachmentOffset.Matrix == App.Matrix())
        matrix1 = App.Matrix()
        matrix1.A34 = 5  # Pocket floor at Z=5 on base pad.
        self.assertTrue(doc.Sketch001.Placement.Matrix == matrix1)
